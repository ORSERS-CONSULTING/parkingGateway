...
async function startServer() {
  ...
  // Set env values after config resolves
  ...
  console.log('IDCS_TENANT:', process.env.IDCS_TENANT);
  console.log('APEX_URL:', process.env.APEX_URL);

  // Require after env is ready
  ...
  /**********************************************************************
   * 4) Route for allowing car to exit
   *********************************************************************/
  app.post('/confirm-from-db', async (req, res) => {
    ...
    if (!plateLicense || immediatelyLeave === undefined || fee === undefined) {
      return res
        .status(400)
        .send({ error: 'Missing plateLicense, immediatelyLeave, or fee' });
    }
  ...
  GET  /run-sync         – manual backfill\n`);
    ...
    try {
      ...
      console.error(
        '❌ failed to register subscription (will retry on next start):',
